...
import { Search, Plus, MapPin, Instagram, Youtube, CheckCircle2, AlertTriangle, Sparkles, ChevronDown, } from "lucide-react";
// Mock creator data
const creators = [
    {
        id: "1",
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-6">
          ...
          <div className="bg-card rounded-xl p-4 border border-border/60 shadow-card mb-6">
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <input type="text" placeholder="Search by name, niche, or handle..." className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-secondary border-0 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50"/>
              </div>
              <div className="flex flex-wrap gap-2">
                {["Instagram", "YouTube", "Trust 70+", "Wellness", "Tech"].map((filter) => (<button key={filter} onClick={() => toggleFilter(filter)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeFilters.includes(filter)
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            return (<button key={creator.id} onClick={() => setSelectedId(creator.id)} className={`w-full text-left p-4 rounded-xl border transition-all ${selectedId === creator.id
              ...
              {selectedId ? (<>
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div className="bg-card rounded-2xl p-6 border border-border/60 shadow-card">
                      ...
                      <div className="space-y-3">
                        {selectedCreator.positives.map((item) => (<InsightCard key={item.title} title={item.title} description={item.description} variant="positive"/>))}
                      </div>
                    </div>
                    <div className="bg-card rounded-2xl p-6 border border-border/60 shadow-card">
                      <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
                      ...
                      <div className="space-y-3">
                        {selectedCreator.risks.map((item) => (<InsightCard key={item.title} title={item.title} description={item.description} variant="warning"/>))}
                      </div>
                    </div>
                  </div>
